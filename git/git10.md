...
远程引用的分支（位于`refs/heads`目录下的引用）之间最主要的区别在于，远程引用是只读的。虽然可以`git checkout`到某个远程引用，但是Git并不会将HEAD引用指向远程引用。因此，你永远不能通过`commit`命令来更新远程引用。git将这些远程引进作为记录远程服务器上各分支最后已知位置状态的书签来管理。
